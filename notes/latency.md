...
Latency exhibits both stable as well as highly variable behavior depending on its componenets:
1. Propagation Dealy: primarily dependent on physical distance and medium; largely consatant for fixed netwrok path
2. Transmission Delay: depends on packet size and link badnwidth; predictable and bounded under stable bandwidth conditions
3. Queuing Delay: depends on traffic load and congestion; highly variable and often responsible for latency spike
...
Latnecy is rarely observed at component level. Instead, it is usually inferred from Round Trip Time (RTT) measurements using tools such as `ping`.
Important implication:
- indivisual delay components are not directly measurable
- observed latency is a collapsed signal, hiding multiple underlying causes
- any attemp to analyze or predict latency must relay on inference, and not direct measurement
...
- hardware and software processign overhead

## Relevance of Machine Learning Modeling:
From machine learning perspective, latency components differ in their lernability:
1. Propagation Delay:
...
Effective latency modeling therefore require:
- analytical estimation of deterministic components
- ML-based inference of dynamic components

## Role of Latency in this Project:
In context of **Internt Latency Decomposition Model**:
- latency serves as the observable agregate signal
- the goal is not merely to predict latency, but to:
    ...
    - explain latency spike rather than just measuring them
